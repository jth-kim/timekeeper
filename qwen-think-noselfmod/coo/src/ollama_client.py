...
from config import (
    OLLAMA_BASE_URL, OLLAMA_MODEL, OLLAMA_THINK,
    OLLAMA_NUM_CTX, OLLAMA_READ_TIMEOUT, CONFIG_DIR, SELF_MOD_ENABLED,
)
...
def _load_system_prompt(inner: bool = False) -> str:
    ...
    if inner:
        ...
        if prompt_path.exists():
            ...
            if inner and not SELF_MOD_ENABLED:
                role_prompt += (
                    "\n\n## Runtime Constraint\n"
                    "Self-modification is DISABLED for this experiment run. "
                    "Do not propose or attempt config file edits. In evolve mode, "
                    "you must return `config_edits` as an empty list."
                )
...
def _strip_thinking(raw: str) -> str:
    """Remove <think>...</think> blocks from Ollama thinking mode responses."""
    return re.sub(r'<think>.*?</think>\s*', '', raw, flags=re.DOTALL)


def _parse_json_response(raw: str) -> dict:
    """Parse a JSON response from the LLM, handling markdown fences and thinking tags."""
...
{memory_context if memory_context else "No memory yet — this is early in your operation."}"""
    ...
    # Thinking mode needs much more token budget for the reasoning trace
    if OLLAMA_THINK:
        max_tokens = 16384 if mode in _INNER_MODES else 4096
    else:
...
If switching, respond with just the new mode name. If the scheduled mode is fine, respond with: {default_mode}"""
    ...
    mode = _strip_thinking(raw).strip().lower().rstrip(".")
